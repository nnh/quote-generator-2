...
class SetValuesRegistrationSheet extends SetValuesSheetByYear{
  ...
  editValues(year){
    ...
    const itemNameAndCount = [
      ...
      ['開始前モニタリング・必須文書確認', Number.isSafeInteger(this.inputData.get('1例あたりの実地モニタリング回数')) ? this.getDivisionCount_(this.inputData.get('1例あたりの実地モニタリング回数') * trialInfo.get('cases'), year): null],
      ['症例モニタリング・SAE対応', Number.isSafeInteger(this.inputData.get('年間1施設あたりの必須文書実地モニタリング回数')) ? this.getDivisionCount_(this.inputData.get('年間1施設あたりの必須文書実地モニタリング回数') * trialInfo.get('facilities') * trialInfo.get('registrationYearsCount'), year): null],
...
class SetValuesSetupSheet extends SetValuesSheetByYear{
  editValues(){
    ...
    const itemNameAndCount = [
      ...
      ['モニタリング準備業務（関連資料作成、キックオフ参加）', Number.isSafeInteger(this.inputData.get('1例あたりの実地モニタリング回数')) ? 1 : null],
